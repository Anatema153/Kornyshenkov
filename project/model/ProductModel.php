...
Class ProductModel extends Dbh
{
	...
	public function p_add_row()
	{
		...
		if(!empty($_REQUEST['add_Id_product'])
			AND !empty($_REQUEST['add_Name_produkt'])
											...
											AND !empty($_REQUEST['add_Id_postavshik']))
		{
			...
			$sql = "INSERT INTO produkti(`Id_product`,`Name_produkt`,`Kolichestvo`,`Kalorii`,`Id_podacategory`,`Srok_postavki`,`Srok_godnosty`,`Srok_realisats`,`Uslovia`,`Id_postavshik`)
			VALUE (?,?,?,?,?,?,?,?,?,?)";
			$stmt=$this->getDbh()->prepare($sql);
			$stmt->execute(array($add_Id_product,$add_Name_produkt,$add_Id_podacategory,$add_Kolichestvo,$add_Kalorii,$add_Srok_postavki,$add_Srok_godnosty,$add_Srok_realisats,$add_Uslovia,$add_Id_postavshik));
			return true;
		}
		else
		{
